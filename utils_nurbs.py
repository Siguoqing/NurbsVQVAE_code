...
def get_se_args():
    ...
    parser.add_argument('--data_list', type=str, default='data/deepcad_data_split_6bit.pkl', help='Path to data folder')
    parser.add_argument('--surface_list', type=str, default='data/deepcad_parsed_unique_surfaces.pkl',
                        help='Path to deduplicated surface data file')
    parser.add_argument('--edge_list', type=str, default='data/deepcad_parsed_unique_edges.pkl',
                        help='Path to deduplicated edge data file')
    
    # Training parameters
    parser.add_argument('--batch_size', type=int, default=2048, help='input batch size')
    parser.add_argument('--train_nepoch', type=int, default=10000, help='number of epochs to train for')
    parser.add_argument('--test_nepoch', type=int, default=1, help='number of epochs to test model')
    ...
    parser.add_argument('--max_edge', type=int, default=124, help='maximum number of edges per face')
    parser.add_argument('--save_folder', type=str, default='my_data', help='save folder')
    parser.add_argument("--gpu", type=int, nargs='+', default=[0], help="GPU IDs to use for training. If None, a random available GPU will be selected.")
    parser.add_argument('--weight', type=str, default='checkpoint/se/abc/8192,4096,128,64,false,1e-4,0,p/abc_se_vqvae_epoch_720.pt', help='指定checkpoint文件路径。留空时从头开始训练')
    
    # === VQ-VAE模型配置选项 ===
    parser.add_argument('--use_type_flag', action='store_true', default=False,
                        help='是否使用类型标志位。True: 4通道输入(3坐标+1标志), False: 3通道输入(仅坐标)')
    ...
    parser.add_argument('--env', type=str, default="se/abc/8192,4096,128,64,false,1e-4,0,p", help='environment')
    ...
    parser.add_argument('--tb_log_dir', type=str, default="logs/se/abc/8192,4096,128,64,false,1e-4,0,p", help='TensorBoard log directory path.')
...
def get_edge_args():
    ...
    parser.add_argument('--train_nepoch', type=int, default=10000, help='number of epochs to train for')
    parser.add_argument('--test_nepoch', type=int, default=20, help='number of epochs to test model')
    parser.add_argument('--save_nepoch', type=int, default=50, help='number of epochs to save model')
    parser.add_argument('--save_folder', type=str, default='my_data', help='save folder')
    parser.add_argument("--gpu", type=int, nargs='+', default=[0], help="GPU IDs to use for training. If None, a random available GPU will be selected.")
    parser.add_argument('--weight', type=str, default='checkpoint/edge/deepcad/2/edge_vqvae_epoch_300.pt', help='指定checkpoint文件路径。留空时从头开始训练')
    ...
    # Save dirs and reload
    parser.add_argument('--env', type=str, default="edge/deepcad/2", help='environment')
    parser.add_argument('--dir_name', type=str, default="checkpoint", help='name of the log folder.')
    parser.add_argument('--loss_dir', type=str, default="./", help='name of the loss folder.')
    parser.add_argument('--tb_log_dir', type=str, default="logs/edge/deepcad/2", help='TensorBoard log directory path.')
    args, _ = parser.parse_known_args()
    # saved folder
    args.save_dir = f'{args.dir_name}/{args.env}'
    return args
...
def get_ar_args():
    parser = argparse.ArgumentParser()
    # 1. 【核心修正】精确指向我们刚刚生成的终极教材！
    parser.add_argument('--sequence_file', type=str, default='data/deepcad_nurbs_sequences_vqvae.pkl',
                        help='Path to preprocessed AR sequences')

    # Training parameters - 优化后的参数
    parser.add_argument('--batch_size', type=int, default=32, help='input batch size - increased for better stability')
    parser.add_argument('--train_nepoch', type=int, default=10000, help='number of epochs to train for')
    parser.add_argument('--test_nepoch', type=int, default=1, help='number of epochs to validate model')
    parser.add_argument('--save_nepoch', type=int, default=10, help='number of epochs to save model')
    parser.add_argument('--max_face', type=int, default=50, help='maximum number of faces')
    parser.add_argument('--max_edge', type=int, default=124, help='maximum number of edges per face')
    parser.add_argument('--save_folder', type=str, default='my_data', help='save folder')

    # 2. 【核心修正】留空！我们是从头开始训练全新的 LLaMA，不要加载旧残骸
    parser.add_argument('--weight', type=str, default='', help='指定checkpoint文件路径。留空时从头开始训练')

    parser.add_argument('--max_seq_len', type=int, default=2048, help='Maximum sequence length')

    # === 防过拟合参数 ===
    parser.add_argument('--learning_rate', type=float, default=1e-3, help='学习率 (降低以减少过拟合)')
    parser.add_argument('--weight_decay', type=float, default=0.01, help='L2权重衰减系数 (增强正则化)')
    parser.add_argument('--dropout', type=float, default=0.1, help='Dropout概率 (增强正则化)')
    parser.add_argument('--label_smoothing', type=float, default=0.0, help='标签平滑系数 (增强正则化)')

    # === 模型架构参数 ===
    parser.add_argument('--d_model', type=int, default=256, help='模型维度 (从768降低到256)')
    parser.add_argument('--nhead', type=int, default=8, help='注意力头数 (从12降低到8)')
    parser.add_argument('--num_layers', type=int, default=8, help='Transformer层数 (从12降低到8)')
    parser.add_argument('--dim_feedforward', type=int, default=1024, help='前馈网络维度 (从3072降低到1024)')

    # 3. 【核心修正】统一环境名称为 deepcad
    parser.add_argument('--dataset_type', type=str, choices=['furniture', 'deepcad', 'abc'],
                        default='deepcad', help='Dataset type to use')

    # Save dirs and reload
    parser.add_argument('--save_dir', type=str, default="checkpoints/ar/deepcad", help='name of the log folder.')
    parser.add_argument('--loss_dir', type=str, default="./", help='name of the loss folder.')
    parser.add_argument('--tb_log_dir', type=str, default="logs/ar/deepcad", help='name of the tensorboard log folder.')
    args = parser.parse_args()

    # saved folder
    args.save_dir = f'{args.save_dir}/{args.dataset_type}'
    return args

def get_nurbs_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_list', type=str, default='data/deepcad_nurbs_data_split_6bit.pkl', help='Path to data folder')
    parser.add_argument('--surface_nurbs_list', type=str, default='data/deepcad_nurbs_data_split_6bit_face.pkl',
                        help='Path to deduplicated surface data file')
    parser.add_argument('--edge_nurbs_list', type=str, default='data/deepcad_nurbs_data_split_6bit_edge.pkl',
                        help='Path to deduplicated edge data file')
    
    # Training parameters
    parser.add_argument('--batch_size', type=int, default=4096, help='input batch size')
    parser.add_argument('--train_nepoch', type=int, default=10000, help='number of epochs to train for')
    parser.add_argument('--test_nepoch', type=int, default=1, help='number of epochs to text2hidden.py model')
    parser.add_argument('--save_nepoch', type=int, default=50, help='number of epochs to save model')
    parser.add_argument('--max_face', type=int, default=50, help='maximum number of faces')
    parser.add_argument('--max_edge', type=int, default=113, help='maximum number of edges per face')
    parser.add_argument('--save_folder', type=str, default='my_data', help='save folder')
    parser.add_argument("--gpu", type=int, nargs='+', default=[0], help="GPU IDs to use for training. If None, a random available GPU will be selected.")
    parser.add_argument('--weight', type=str, default='checkpoint/se/abc/8192,4096,128,64,false,1e-4,0,p/abc_se_vqvae_epoch_720.pt', help='指定checkpoint文件路径。留空时从头开始训练')
    ...
    parser.add_argument('--dataset_type', type=str, choices=['furniture', 'deepcad', 'abc'],
                        default='deepcad', help='Dataset type to use')
    parser.add_argument('--vqvaetype', type=str, choices=['se', 'nurbs'], default='nurbs',
                        help='VQVAE类型，用于权重命名')
    parser.add_argument('--quantization_size', type=int, default=4096, help='VQ codebook size')
    parser.add_argument('--model_down_blocks', type=int, default=1,
                        help='Number of encoder/decoder blocks. 1 keeps a 4x4 latent grid, 2 compresses to 2x2.')
    parser.add_argument('--base_channel_dim', type=int, default=64, help='Base width for VQ-VAE blocks')
    parser.add_argument('--latent_channels', type=int, default=128, help='Latent channels before quantization')
    parser.add_argument('--vq_embed_dim', type=int, default=64, help='Embedding dimension used by the codebook')
    parser.add_argument('--learning_rate', type=float, default=1e-4, help='VQ-VAE learning rate')
    parser.add_argument('--weight_decay', type=float, default=1e-6, help='AdamW weight decay')
    parser.add_argument('--beta1', type=float, default=0.9, help='AdamW beta1')
    parser.add_argument('--beta2', type=float, default=0.999, help='AdamW beta2')
    parser.add_argument('--max_grad_norm', type=float, default=1.0, help='Gradient clipping norm, 0 to disable')
    parser.add_argument('--recon_l1_weight', type=float, default=0.5, help='Extra L1 term on all control points')
    parser.add_argument('--face_boundary_weight', type=float, default=2.0, help='Extra L1 weight on face boundaries')
    parser.add_argument('--face_corner_weight', type=float, default=4.0, help='Extra L1 weight on face corners')
    parser.add_argument('--edge_endpoint_weight', type=float, default=2.0, help='Extra L1 weight on edge endpoints')
    parser.add_argument('--max_error_weight', type=float, default=0.15, help='Penalty on batch mean max abs error')
    parser.add_argument('--boundary_max_error_weight', type=float, default=0.3, help='Penalty on boundary max abs error')
    parser.add_argument('--vq_loss_weight', type=float, default=1.0, help='Weight applied to VQ loss')
    parser.add_argument('--vq_distance', type=str, choices=['cos', 'l2'], default='cos',
                        help='Distance metric used by vector quantizer')
    parser.add_argument('--vq_anchor', type=str, choices=['probrandom', 'closest', 'random'], default='probrandom',
                        help='Anchor update strategy for vector quantizer')
    parser.add_argument('--vq_first_batch', action='store_true', default=False,
                        help='Freeze codebook reinit after first batch')
    parser.add_argument('--vq_contras_loss', type=lambda x: str(x).lower() in ('1', 'true', 'yes', 'y'),
                        default=True, help='Whether to keep contrastive codebook loss')
    parser.add_argument('--vq_beta', type=float, default=None, help='Override beta used by vector quantizer')

    # Save dirs and reload
    parser.add_argument('--env', type=str, default="se/abc/8192,4096,128,64,false,1e-4,0,p", help='environment')
    parser.add_argument('--dir_name', type=str, default="checkpoint", help='name of the log folder.')
    parser.add_argument('--loss_dir', type=str, default="./", help='name of the loss folder.')
    parser.add_argument('--tb_log_dir', type=str, default="logs/nurbs/CVQ/1024,4096,(64,128),128,3,1e-3,5,p", help='name of the loss folder.')
...
def check_brep_validity(shape: TopoDS_Shape) -> bool:
    """检查B-rep模型是否满足所有有效性标准
    Args:
        shape: 待检查的B-rep模型
    Returns:
        bool: 是否通过所有检查（True=有效，False=无效）
    """
    try:
        # 使用OpenCASCADE的内置检查器
...
# 1. 可三角化检查 (Triangulatable)
def check_triangulatable(shape: TopoDS_Shape) -> bool:
    """检查模型所有面片是否可生成三角形
    标准1: B-rep中的每个面片必须至少生成一个三角形
    """
    ...
    while explorer.More():
        ...
        # 检查是否生成了至少一个三角形
        triangulation = OCC.Core.BRep.BRep_Tool.Triangulation(face, 0.1)
...
# 2. 环顺序检查 (Wire ordering)
def check_wire_ordering(shape: TopoDS_Shape) -> bool:
    """检查所有环的边顺序是否正确
    标准2: B-rep中每个环的边必须顺序正确
    使用ShapeAnalysis_Wire::CheckOrder()检查
    """
    ...
    while explorer.More():
        ...
        wire_data = ShapeExtend_WireData(wire)
        analyzer.Load(wire_data)
        
        # 检查环顺序（使用指定容差）
        if not analyzer.CheckOrder(0.1):
            return False
...
# 3. 环自相交检查 (No wire self-intersection)
def check_wire_self_intersection(shape: TopoDS_Shape) -> bool:
    """检查是否有环自相交
    标准3: 环不应自相交
    使用ShapeAnalysis_Wire::CheckSelfIntersection()检查
    """
    explorer = TopExp_Explorer(shape, TopAbs_WIRE)
    while explorer.More():
        wire = TopoDS_Wire(explorer.Current())
...
# 4. 坏边检查 (No bad edges)
def check_shell_edges(shape: TopoDS_Shape) -> bool:
    """检查壳中是否有坏边
    标准4: 壳中的边应出现一次或两次，但方向不同
    使用ShapeAnalysis_Shell::HasBadEdges()检查
    """
    # 查找模型中的所有壳
    shells = []
    explorer = TopExp_Explorer(shape, TopAbs_SHELL)
    while explorer.More():
        shells.append(TopoDS_Shell(explorer.Current()))
        explorer.Next()
    
    # 如果没有找到壳，直接返回True（视为通过）
    if not shells:
        return True
...
def load_se_vqvae_model(vqvae_model_path, use_type_flag, device):
    
    try:
        in_channels = 4 if use_type_flag else 3
        
        vqvae_model = VQVAE(
            in_channels=in_channels,  # 根据标志位设置输入通道数
            out_channels=3,
...
def quantize_hybrid_nonlinear(coords, num_tokens=2048, exponent=0.9):
    """
    混合非线性量化函数。
    - 为 -1, 0, 1 分配专属索引。
    - 对 (-1, 1) 区间使用幂函数进行非线性量化，在0附近更精细。

    支持 numpy.ndarray 或 torch.Tensor 输入；返回类型与输入一致。
    
    Args:
        coords (np.ndarray | torch.Tensor): 输入的坐标数据，建议范围在 [-1, 1]。
        num_tokens (int): 总的量化级别数。
        exponent (float): 幂函数的指数 (0 < exponent < 1)。越小，0附近越精细。
    """
    # 确保 num_tokens 是偶数，以便对称划分
    assert num_tokens % 2 == 0, "num_tokens should be even for symmetric quantization."

    # 统一为torch进行计算
    input_is_numpy = isinstance(coords, np.ndarray)
    if input_is_numpy:
        coords_t = torch.from_numpy(coords).float()
    elif isinstance(coords, torch.Tensor):
        coords_t = coords.float()
    else:
        raise TypeError(f"Unsupported coords type: {type(coords)}. Expected numpy.ndarray or torch.Tensor.")

    # 初始化结果张量
    quantized = torch.zeros_like(coords_t, dtype=torch.long)
    
    # --- 1. 处理三个特殊值 ---
    TOKEN_NEG_ONE = 0
    TOKEN_POS_ONE = num_tokens - 1
    TOKEN_ZERO = num_tokens // 2 - 1 # 例如，2048 -> 1023

    quantized[coords_t == -1.0] = TOKEN_NEG_ONE
    quantized[coords_t == 1.0] = TOKEN_POS_ONE
    quantized[coords_t == 0.0] = TOKEN_ZERO
    
    # --- 2. 处理正数区间 (0, 1) ---
    pos_mask = (coords_t > 0) & (coords_t < 1.0)
    if pos_mask.any():
        pos_coords = coords_t[pos_mask]
        
        # 非线性变换: y = x^exponent
        transformed_pos = torch.pow(pos_coords, exponent)
        
        # 线性映射到正数区间的 token 范围
        # 正数 tokens: [TOKEN_ZERO + 1, TOKEN_POS_ONE - 1]
        # 例如: [1024, 2046]，共 1023 个
        num_pos_bins = (TOKEN_POS_ONE - 1) - (TOKEN_ZERO + 1) + 1
        
        scaled_pos = transformed_pos * (num_pos_bins - 1)
        quantized_pos = torch.round(scaled_pos).long() + (TOKEN_ZERO + 1)
        quantized[pos_mask] = quantized_pos

    # --- 3. 处理负数区间 (-1, 0) ---
    neg_mask = (coords_t > -1.0) & (coords_t < 0)
    if neg_mask.any():
        neg_coords = coords_t[neg_mask]
        
        # 对绝对值进行非线性变换: y = |x|^exponent
        transformed_neg = torch.pow(-neg_coords, exponent)
        
        # 线性映射到负数区间的 token 范围
        # 负数 tokens: [TOKEN_NEG_ONE + 1, TOKEN_ZERO - 1]
        # 例如: [1, 1022]，共 1022 个
        num_neg_bins = (TOKEN_ZERO - 1) - (TOKEN_NEG_ONE + 1) + 1

        # 注意：我们希望接近0的值（transformed_neg接近0）映射到接近TOKEN_ZERO的索引
        # 所以我们需要翻转映射范围
        scaled_neg = (1.0 - transformed_neg) * (num_neg_bins - 1)
        quantized_neg = torch.round(scaled_neg).long() + (TOKEN_NEG_ONE + 1)
        quantized[neg_mask] = quantized_neg
...
def dequantize_hybrid_nonlinear(indices, num_tokens=2048, exponent=0.9):
    """
    对应的混合非线性反量化函数。

    支持 numpy.ndarray 或 torch.Tensor 输入；返回类型与输入一致。
    """
    assert num_tokens % 2 == 0, "num_tokens should be even."

    input_is_numpy = isinstance(indices, np.ndarray)
    if input_is_numpy:
        indices_t = torch.from_numpy(indices).long()
    elif isinstance(indices, torch.Tensor):
        indices_t = indices.long()
    else:
        raise TypeError(f"Unsupported indices type: {type(indices)}. Expected numpy.ndarray or torch.Tensor.")

    coords = torch.zeros_like(indices_t, dtype=torch.float32)
    
    # --- 1. 处理三个特殊值 ---
    TOKEN_NEG_ONE = 0
    TOKEN_POS_ONE = num_tokens - 1
    TOKEN_ZERO = num_tokens // 2 - 1

    coords[indices_t == TOKEN_NEG_ONE] = -1.0
    coords[indices_t == TOKEN_POS_ONE] = 1.0
    coords[indices_t == TOKEN_ZERO] = 0.0
    
    # --- 2. 处理正数区间 ---
    pos_mask = (indices_t > TOKEN_ZERO) & (indices_t < TOKEN_POS_ONE)
    if pos_mask.any():
        pos_indices = indices_t[pos_mask]
        
        num_pos_bins = (TOKEN_POS_ONE - 1) - (TOKEN_ZERO + 1) + 1
        
        # 从 token 索引反向映射回 [0, 1] 的非线性空间
        normalized = (pos_indices.float() - (TOKEN_ZERO + 1)) / (num_pos_bins - 1)
        
        # 应用反函数: x = y ^ (1/exponent)
        dequantized_pos = torch.pow(normalized, 1.0 / exponent)
        coords[pos_mask] = dequantized_pos
        
    # --- 3. 处理负数区间 ---
    neg_mask = (indices_t > TOKEN_NEG_ONE) & (indices_t < TOKEN_ZERO)
    if neg_mask.any():
        neg_indices = indices_t[neg_mask]
        
        num_neg_bins = (TOKEN_ZERO - 1) - (TOKEN_NEG_ONE + 1) + 1
        
        # 从 token 索引反向映射，同样注意翻转
        normalized = (neg_indices.float() - (TOKEN_NEG_ONE + 1))
        reversed_normalized = (num_neg_bins - 1 - normalized) / (num_neg_bins - 1)
        
        # 应用反函数并添加负号
        dequantized_neg = -torch.pow(reversed_normalized, 1.0 / exponent)
        coords[neg_mask] = dequantized_neg
...
def prepare_vqvae_input(data_ncs, data_type='face', use_type_flag=False, device="cpu"):
    ...
    此函数将不同类型（面、边、边界框）的NCS/WCS数据转换为
    VQ-VAE模型所需的标准张量格式 (B, C, H, W)。

    - bbox部分已根据Dataset中的复制方法进行修改，仅支持(B, 6)输入。
    
    Args:
        data_ncs (np.ndarray or list): 输入的几何数据。
            - 对于 'face': 形状为 (B, 32, 32, 3)。
            - 对于 'edge': 形状为 (B, 32, 3)。
            - 对于 'bbox': 形状严格为 (B, 6)。
        data_type (str): 数据类型，可选值为 'face', 'edge', 'bbox'。
        use_type_flag (bool): 是否为面和边数据添加类型标志。
        device (str): 目标设备，例如 "cpu" 或 "cuda"。

    Returns:
        torch.Tensor: 格式化后的PyTorch张量。
    """
    ...
    if data_type == 'face':
        ...
        for i in range(batch_size):
            ...
            # --- 以下是您指定的、未作任何修改的NumPy代码 ---
            # 将6维向量转换为(8,8,3)张量 - 使用高效v1方法
