...
class VectorQuantiser(nn.Module):
    ...
    def _safe_probabilistic_sample(self, distances, features):
        sample_logits = torch.nan_to_num(
            distances.t().float(),
            nan=-1e4,
            posinf=1e4,
            neginf=-1e4,
        )
        norm_distance = F.softmax(sample_logits, dim=1)
        norm_distance = torch.nan_to_num(norm_distance, nan=0.0, posinf=0.0, neginf=0.0)
        row_sums = norm_distance.sum(dim=1, keepdim=True)
        invalid_rows = row_sums.squeeze(1) <= 0

        if invalid_rows.any():
            norm_distance[invalid_rows] = 1.0
            row_sums = norm_distance.sum(dim=1, keepdim=True)

        norm_distance = norm_distance / row_sums.clamp_min(1e-12)
    ...
    def forward(self, z, temp=None, rescale_logits=False, return_logits=False):
        ...
        z_flattened = self._sanitize_features(z.view(-1, self.embed_dim))
        ...
        d = torch.nan_to_num(d.float(), nan=-1e4, posinf=1e4, neginf=-1e4)
        ...
        if self.training:
            ...
            if self.anchor in ['closest', 'random', 'probrandom'] and (not self.init):
                ...
                # probabilitical based random sampling
                    ...
                    random_feat = self._safe_probabilistic_sample(d, z_flattened.detach().float())
